...
impl Edid {
    /// Helper method that returns the number of block map extensions required when `n_extensions`
    /// need to be added.
    fn number_of_block_map_extensions(n_extensions: usize) -> usize {
        match n_extensions {
            0..=1 => 0,
            2..=126 => 1,
            127..=252 => 2,
            _ => panic!("Too many extensions ({n_extensions} vs maximum expected 252)"),
        }
    }
    ...
    fn extensions_into_bytes(extensions: Vec<EdidExtension>) -> Vec<u8> {
        let num_block_map_exts = Edid::number_of_block_map_extensions(extensions.len());
        let mut bytes = Vec::with_capacity(
            extensions.len() * EDID_BASE_LEN + num_block_map_exts * EDID_BASE_LEN,
        );
...
impl IntoBytes for Edid {
    fn into_bytes(self) -> Vec<u8> {
        ...
        let num_block_map_exts = Edid::number_of_block_map_extensions(self.extensions.len());
        let num_exts = (self.extensions.len() + num_block_map_exts)
            .to_u8()
...
mod test_edid {
        ...
        for _ in 0..num_extensions {
            extensions.push(EdidExtension::Manufacturer(
                EdidExtensionManufacturer::builder()
                    ...
                    .expect("Vendor data should be valid")
                    .build(),
            ));
        }
    ...
    #[test]
    fn test_number_of_block_map_extensions() {
        assert_eq!(Edid::number_of_block_map_extensions(0), 0);
        assert_eq!(Edid::number_of_block_map_extensions(1), 0);
        assert_eq!(Edid::number_of_block_map_extensions(2), 1);
        assert_eq!(Edid::number_of_block_map_extensions(126), 1);
        assert_eq!(Edid::number_of_block_map_extensions(127), 2);
        assert_eq!(Edid::number_of_block_map_extensions(252), 2);
    }

    #[test]
    #[should_panic(expected = "Too many extensions (253 vs maximum expected 252)")]
    fn test_number_of_block_map_extensions_too_many() {
        Edid::number_of_block_map_extensions(253);
    }
    ...
    fn test_extensions_into_bytes_with_1_extension() {
        let bytes = generate_extensions(1);
